...
docker run --network=sample-network --rm -v $root:/sitespeed.io sitespeedio/sitespeed.io:20.2.0 \
...
echo 'public urls run'
 docker run --network=sample-network --rm -v $root:/sitespeed.io sitespeedio/sitespeed.io:20.2.0-plus1 \
 --graphite.host=graphite-sample publicUrls.txt -b edge -n 1