...
public class ItemServiceImpl implements ItemsService {
  ...
  public List<ItemModel> getPublicPublishedItems(Pageable pageable) {
    return itemRepository.findByIsPublicTrueAndStatus(pageable, ItemStatus.DONE).stream()
        .map(this::getModel)
        ...
        .orElseThrow(
            () ->
                new EmptyResultDataAccessException(
                    String.format("Item with id %s not found", uuid), 1));
  }
  ...
  private ItemModel getModel(Items item) {
    return ItemModel.builder()
        ...
        .publiclyAvailable(item.isPublic())
