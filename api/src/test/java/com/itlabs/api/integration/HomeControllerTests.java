...
public class HomeControllerTests extends BaseIntegration {
	...
	void itemsAuthorizedGetTest() throws Exception {
		sampleData();
		final var resultActions = mvc.perform(getAuthorizationBuilder(get(Routes.HOME_ROUTE + "/items"))).andDo(print())
	...
	void itemsUnAuthorizedGetTest() throws Exception {
		sampleData();
		final var resultActions = mvc.perform(get(Routes.HOME_ROUTE + "/items")).andDo(print())
				.andExpect(status().isOk());
	...
	private void validate(ResultActions resultActions) throws Exception {
		...
		for (int i = 0; i < 5; i++) {
			String id = "$[" + i + "].";
			resultActions.andExpect(jsonPath(id + "publiclyAvailable").value("true"))
	...
	private void sampleData() {
		...
		this.seedItemsInDatabase(15).stream().skip(6).forEach(x -> {
			ItemEditModel model = new ItemEditModel(x.getName(), ItemStatus.DONE, x.getDescription(), x.isPublic());
			itemsService.update(x.getGuid(), model);
		});
		;
		this.seedItemsInDatabase(itemsCount, true).stream().skip(3).forEach(x -> {
			ItemEditModel model = new ItemEditModel(x.getName(), ItemStatus.DONE, x.getDescription(), x.isPublic());
			itemsService.update(x.getGuid(), model);
		});
	}
