...
public class ItemsControllerTests extends BaseIntegration {
  ...
  void itemsGetTest() throws Exception {
    ...
    final var resultActions =
        mvc.perform(getAuthorizationBuilder(get(Routes.ITEMS_ROUTE)))
            .andDo(print())
    ...
    this.seedItemsInDatabase(itemsCount)
        .forEach(
            x -> {
              var itemModel =
                  new ItemEditModel(
                      namePrefix + x.getName(), x.getStatus(), x.getDescription(), x.isPublic());
              itemsService.update(x.getGuid(), itemModel);
            });

    final String itemsRoute = Routes.ITEMS_ROUTE + "?name=" + namePrefix;
    final var resultActions =
        mvc.perform(getAuthorizationBuilder(get(itemsRoute)))
            .andDo(print())
            .andExpect(status().isOk());
    ...
    this.seedItemsInDatabase(itemsCount)
        .forEach(
            x -> {
              var itemModel =
                  new ItemEditModel(
                      x.getName(), ItemStatus.IN_PROGRESS, x.getDescription(), x.isPublic());
              itemsService.update(x.getGuid(), itemModel);
            });
  ...
  private void validateContains(
      ResultActions resultActions, String field, int count, String expectedValue) throws Exception {
  ...
  void itemsPostSuccessTest() throws Exception {
    ItemEditModel model =
        new ItemEditModel("testPost", ItemStatus.DRAFT, "test description", false);

    final var resultActions =
        mvc.perform(
                getAuthorizationBuilder(post(Routes.ITEMS_ROUTE))
  ...
  void itemsPostEmptyBodyTest() throws Exception {
    final var resultActions =
        mvc.perform(getAuthorizationBuilder(post(Routes.ITEMS_ROUTE)).content("")).andDo(print());
  ...
  void itemPublishSuccessTest() throws Exception {
    ...
    final var resultActions =
        mvc.perform(
                getAuthorizationBuilder(put(getEditUrl(saved.getId())))
  ...
  void itemUnPublishSuccessTest() throws Exception {
    var model =
        new ItemEditModel(
            "itemUnPublishSuccessTest", ItemStatus.DRAFT, "test edit description", false);
    var saved = itemsService.save(model);
    itemsService.update(
        java.util.UUID.fromString(saved.getId()),
        new ItemEditModel(
            "itemUnPublishSuccessTest ", ItemStatus.DONE, "modified description", false));
    var edit =
        new ItemEditModel(
            "itemUnPublishSuccessTest ", ItemStatus.IN_PROGRESS, "modified description", false);
    final var resultActions =
        mvc.perform(
                getAuthorizationBuilder(put(getEditUrl(saved.getId())))
                    .content(Utils.asJsonString(edit)))
  ...
  void itemsDeleteSuccessTest() throws Exception {
    ...
    var saved = itemsService.save(model);
    final var resultActions =
        mvc.perform(getAuthorizationBuilder(delete(getEditUrl(saved.getId())))).andDo(print());
  ...
  void itemsDeleteWrongIdTest() throws Exception {
    ...
    final var resultActions =
        mvc.perform(
                getAuthorizationBuilder(delete(getEditUrl(java.util.UUID.randomUUID().toString()))))
            .andDo(print());
    resultActions.andExpect(status().isNoContent());
  }
  ...
  void itemsWrongIdTypeTest() throws Exception {
    ...
    final var resultActions =
        mvc.perform(getAuthorizationBuilder(delete(getEditUrl("789878hg")))).andDo(print());
