...
public class HealthMetricsConfiguration {
	...
	public HealthMetricsConfiguration(MeterRegistry registry, Map<String, HealthContributor> healthContributors,
	...
	private void setMeterStatusFromReactiveHealthContributors(
			Map<String, ReactiveHealthContributor> reactiveHealthContributors) {
		reactiveHealthContributors.forEach((name, contributor) -> {
			if (contributor instanceof ReactiveHealthIndicator) {
				...
				healthContributor.forEach(x -> setHealthStatusInMeterRegistry(x.getName(),
						((ReactiveHealthIndicator) x.getContributor())));
			}
			else {
	...
	private void setMeterStatusFromHealthContributors(Map<String, HealthContributor> healthContributors) {
		healthContributors.forEach((name, contributor) -> {
			if (contributor instanceof HealthIndicator) {
				...
				healthContributor.forEach(
						x -> setHealthStatusInMeterRegistry(x.getName(), (HealthIndicator) x.getContributor()));
			}
			else {
				throwIllegalStateException(name);
			}
	...
	private void setHealthStatusInMeterRegistry(String name, Health health) {
		...
		Gauge.builder(ACTUATOR_HEALTH_METRIC_NAME, health, this::getStatusCode).strongReference(true)
				.tag(ACTUATOR_HEALTH_TAG_SERVICE_NAME, applicationName).tag(ACTUATOR_HEALTH_TAG_INDICATOR_NAME, name)
				.register(registry);
	}
